...
function accountLabel(account: Account) {
  return account.email ?? `Google 账号 · ${account.accountId.slice(-6)}`;
}
          ...
          >
            <label className="flex items-center gap-2 border-b border-base-300 px-3.5 py-3">
              ...
              <input
                autoFocus
                type="search"
                aria-label="搜索媒体资源或账号"
                placeholder="搜索媒体资源或账号…"
                value={search}
            ...
            <div className="max-h-72 overflow-y-auto overscroll-contain p-1.5">
              {loading ? (
                ...
                <div role="alert" className="p-3 text-sm">
                  <p className="text-error">无法加载媒体资源。</p>
                  <button
                    type="button"
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 px-2 py-2 text-xs font-medium text-base-content/55">
                        ...
                        <button
                          ...
                          aria-label={`移除 ${accountLabel(account)}`}
                        >
                          移除账号
                        </button>
                      </div>
                      {account.requiresReconnect ? (
                        <div className="flex flex-wrap items-center justify-between gap-2 px-2 pb-2 text-sm">
                          ...
                          <button
                            ...
                            onClick={onReconnect}
                            aria-label={`重新连接 ${accountLabel(account)}`}
                            disabled={linking}
                            aria-busy={linking}
                          >
                            {linking ? "正在打开 Google…" : "重新连接"}
                          </button>
                        </div>
                      ) : account.unavailable ? (
                        <div className="flex flex-wrap items-center justify-between gap-2 px-2 pb-2 text-sm">
            ...
            <div className="border-t border-base-300 p-1.5">
              ...
              >
                ...
                {linking ? "正在打开 Google…" : "添加 Google 账号"}
              </button>
            </div>
          </div>
        ) : null}
