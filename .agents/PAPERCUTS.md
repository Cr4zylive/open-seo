...
- [ ] `2026-09-15T16:52:00Z` — `cursor` — `get_serp_results` text tables print only rank/domain/title/url, so AI Overview vs People Also Ask vs related searches collapse into blank rows. Agents answering “is there an AI Overview?” have to guess from empty rank-1 items. Add a `type` column to `SERP_ITEM_COLUMNS` in `src/server/mcp/tools/get-serp-results.ts`.
